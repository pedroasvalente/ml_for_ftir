...
from ml4fir.config import (
    EXPERIMENTS_DIR,
    FIGURES_DIR,
    MODELS_DIR,MLFLOW_ARTIFACTS_DIR,
    ...
    for i in range(len(all_best_experiment)):
        ...
        try:
            y_pred, y_prob, metrics, lv_importance = evaluate_model(
                trained_model.get_raw_model(), x_test, y_test, x_train, row_best_experiment["model_type"]
            )
        ...
        for plot in plots_to_do:
            plot_folder = os.path.join(
                FIGURES_DIR, plot, target_to_predict
            )
            if only_best:
            ...
            if plot == "ROC":
                ...
                if not do_principal:
                    logger.info("Skipping Principal Wavenumber plot due to previous error.")
                else:
                ...
                for file in files:
                    if os.path.basename(file) == os.path.basename(saved_plot):
                        ...
                        if os.path.exists(os.path.join(root, file)):
                            ...
                            shutil.copyfile(saved_plot, os.path.join(root, file))
